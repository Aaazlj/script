...
Cron: 20 8,12,20 * * *
----------------------------------------------------------------------------------------------
朴朴超市签到code本 v1.1.0 (YYB-Go-Enhanced 适配版)

功能：自动执行朴朴超市小程序每日签到和互相助力任务，支持多账号执行。
登录 code 通过 YYB-Go-Enhanced 的 /wxapp/getCode 获取，无需手动填 ck。

配置说明：
1. 微信 code 网关：（适配 YYB-Go-Enhanced 应用宝协议网关）
   方式一（推荐，兼容面板 YYB_SERVER 约定）：
     yyb_server                                      必填，YYB-Go-Enhanced 网关地址
     - 每个账号一行，格式：{网关地址}@{账号ID或OpenID}
     - 示例：
         http://127.0.0.1:8000@1
         http://192.168.1.10:8000@owNAxxx...xxx
     - 多账号支持使用 &、英文逗号、中文逗号或换行分隔
     - 脚本会自动对每个条目调用 POST {网关}/wxapp/getCode
     - 请求体：{"app_id": "<小程序appid>", "ref": "账号ID或OpenID"}

   方式二（与旧版 ppcs.js 兼容）：
     wx_server_url                                   必填，YYB-Go-Enhanced 网关地址
     ppcs_openid                                     必填，账号ID或OpenID（多账号用 &、中英文逗号或换行分隔）
     - 请求体：{"app_id": "<小程序appid>", "ref": "账号ID或OpenID"}

2. 账号标识说明：
   - ref 可填账号 ID（控制台显示的本地编号，如 1、2）或 OpenID；
   - OpenID 删除重扫后不变，更推荐长期使用；
...
// 方式一：YYB_SERVER 面板环境变量，每行 {网关地址}@{账号ID或OpenID}
...
// ==================== YYB-Go-Enhanced 网关登录 ====================
// 通过 YYB 网关 /wxapp/getCode → 朴朴 silent_login 获取新 token
// 兼容两种 getCode 返回格式：
//   YYB-Go-Enhanced: HTTP 200 => { openid, account, result: { code: "0x...", errMsg: "login:ok" } }
//                    失败     => HTTP 非200 { code, msg, data }
//   旧式应用宝网关 : { code: 0, data: { result: { code, errMsg } } }
async function silentLoginViaGateway(ref, logFn, index, wxServerUrl) {
  ...
  let statusCode = codeRes?.statusCode;
  let wxCode = "";

  // YYB-Go-Enhanced 格式：result.code 为字符串（首次返回也带 errMsg）
  if (statusCode === 200 && codeData && typeof codeData?.result?.code === "string" && codeData.result.code) {
    wxCode = codeData.result.code;
    if (codeData.account) {
      let n = codeData.account.nickname || codeData.account.alias || codeData.account.remark || "";
      if (n) log("👤 网关账号: " + n + " (id=" + codeData.account.id + ")");
    }
  } else if (codeData && codeData.code === 0 && codeData.data?.result?.code) {
    // 旧式应用宝网关格式
    wxCode = codeData.data.result.code;
  }

  if (!wxCode) {
    log("❌ getCode 失败[" + statusCode + "]: " + JSON.stringify(codeData));
    return null;
  }
...
class PupuUser extends BaseRequest {
  constructor(cookieString, ref, wxServerUrl) {
    super();
...
// ==================== 账号加载 ====================
// 优先级：
//   1) YYB_SERVER（或 yyb_server）：每行 {网关地址}@{账号ID或OpenID}，多账号用 &、中英文逗号或换行分隔
//   2) 旧格式 wx_server_url + ppcs_openid
function loadAccounts() {
  let entries = [];
  let serverLine = YYB_SERVER.trim();
  if (serverLine) {
    // 解析 host@ref，每个条目独立网关地址
    for (let item of serverLine.split(/[,，&\n]/).map(s => s.trim()).filter(s => s)) {
      let at = item.lastIndexOf("@");
      if (at === -1) {
        CommonUtils.log("⚠️ YYB_SERVER 格式错误（缺少 @，应为 {地址}@{账号ID或OpenID}）: " + item);
        continue;
      }
      let host = item.substring(0, at).trim();
      let ref = item.substring(at + 1).trim();
      if (host && ref) entries.push({ wxServerUrl: host, ref: ref });
    }
  } else {
    ...
    for (let ref of PPCS_OPENID.split(/[,，&\n]/).map(r => r.trim()).filter(r => r)) {
      entries.push({ wxServerUrl: wxServerUrl, ref: ref });
    }
  }
  ...
  for (let e of entries) {
    CommonUtils.userList.push(new PupuUser("", e.ref, e.wxServerUrl));
  }
